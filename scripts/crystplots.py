...
def plot_check(labels,method,run,dir):
	...
	for i in rate:
		...
		#plt.xlim(0,2)
		plt.show()
		plt.savefig(name[param]+"_"+str(run)+".png")
		plt.close()
		
		param+=1
	sub.call("mv *.png "+data_dir, shell=True)
...
def plot_compare(labels,method,run, dir):
	...
	for i in merit:
		...
		plt.savefig(i+"_compare_"+str(run)+".png")
		
		plt.show()
		plt.close()
	sub.call("mv *_compare*.png "+data_dir, shell=True)
