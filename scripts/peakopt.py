...
def finalpeakopt(inp,curves,label,geom,cell,proc):
	count=1
	f=open("output.tab","w+")
	i=0
	...
	while i<(len(curves)):
		#print(i)
		out=f"../indexing/{label}_{i}.stream"
		if curves[i][0]=='zaef':
			my_string="indexamajig -i "+inp+" -g "+geom+" --indexing=xgandalf --no-cell-combinations --multi --min-peaks=14 --peaks=zaef -o "+out+" -j "+str(proc)+" --threshold="+str(curves[i][1])+" --min-squared-gradient="+str(curves[i][2])+" --min-snr="+str(curves[i][3])+" --peak-radius="+str(curves[i][4])+" --profile"
		...
		if curves[i][0]=='peakfinder8':
			my_string="indexamajig -i "+inp+" -g "+geom+" --indexing=xgandalf --no-cell-combinations --multi --min-peaks=14 --peaks=peakfinder8 -o "+out+" -j "+str(proc)+" --threshold="+str(curves[i][1])+" --min-snr="+str(curves[i][2])+" --min-pix-count="+str(curves[i][3])+" --max-pix-count="+str(curves[i][4])+" --local-bg-radius="+str(curves[i][5])+" --profile"
		if cell!=0:
